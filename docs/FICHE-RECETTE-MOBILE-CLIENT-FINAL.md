...
- *(Optionnel)* Se connecter avec ce compte → écran A3.
...
3. Utiliser la **recherche produit**.
...
2. Repérer un versement **en attente** créé par le boucher (D9).
3. Appuyer sur **Valider** → statut **validé**.
4. Sur un autre versement **en attente**, appuyer sur **Rejeter**.
5. Saisir un **motif** si demandé → statut **rejeté**.

**Résultat attendu**

- Le fournisseur peut valider ou rejeter les paiements reçus.
...
1. **Distribution** : sélectionner une ligne (date · kg · produit).
2. **Quantité reçue** : saisir le poids réellement reçu.
3. **Date de réception** : date du jour.
4. **Notes** : optionnel.
5. *(Optionnel)* Note vocale.
6. **Enregistrer**.

**Résultat attendu**

- Message de succès ; stock augmenté (vérifiable en D2).
...
2. Consulter la liste des **mouvements**.

**Résultat attendu**

- Mouvements de réception (D3) et/ou ajustement (D4) visibles.
...
3. **Client** : optionnel.
4. **Produit** : stock suffisant.
5. Vérifier le **prix unitaire** (modifiable).
6. **Quantité vendue** : ex. 2 kg.
7. Vérifier le **total**.
8. **Enregistrer**.
...
1. Comme D6 mais **Type de vente** : Livraison.
2. Choisir un **client** (obligatoire).
3. **Adresse de livraison** : ex. `Quartier Test, rue 12`.
4. **Date prévue** : date future ou du jour.
5. **Enregistrer**.

**Résultat attendu**

- Vente + livraison créées sans erreur.
...
1. Filtrer par dates, type ou statut.
2. Repérer une vente récente (D6 ou D7).
3. Appuyer sur **Payée**.
4. Sur une autre vente, appuyer sur **Annuler** (si autorisé).
...
1. **Fournisseur** : liste ou lecture seule si assigné.
2. **Montant** : ex. 50 000 FCFA.
3. **Mode de paiement** : ex. Mobile money / Espèces.
4. **Date** : aujourd'hui.
5. **Référence** : ex. `VIR-RECETTE-001` (unique).
6. **Notes** : optionnel.
7. *(Optionnel)* Note vocale.
8. **Enregistrer**.

**Résultat attendu**

- Versement **en attente** (visible fournisseur en C6).
...
1. **Rapport de ventes** (comme B7).
2. **Rapport de stocks** (comme B8).
3. **Rapport financier** (comme B9).

**Résultat attendu**

- Données alignées avec ventes et stocks réels.
...
À exécuter **dans l'ordre**, avec les **trois comptes**.
